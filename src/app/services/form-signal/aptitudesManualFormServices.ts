...
export class AptitudesManualFormService extends AbstractSignalFormService<AptitudesManualForm> {
    ...
    private generateCodeBuild(value: AptitudesManualForm): string {
        const values = [
            value.pointDeVie, value.pa, value.pm, value.pw,
            value.maitriseFeu, value.maitriseEau, value.maitriseTerre, value.maitriseAir,
            value.resistancesFeu, value.resistancesEau, value.resistancesTerre, value.resistancesAir,
            value.di, value.soinsRealises, value.cc, value.parade, value.initiative, value.portee,
            value.esquive, value.tacle, value.sagesse, value.prospection, value.volonte,
            value.maitriseCritique, value.resistancesCritique, value.maitriseDos, value.resistancesDos,
            value.maitriseMelee, value.armureDonnee, value.maitriseDistance, value.armureRecue,
            value.maitriseSoins, value.DIindirect, value.maitriseBerzerk
        ];
    ...
    public decodeAndSaveCodeBuild(codeBuild: string): void {
        try {
            const aptitudesManual: AptitudesManualForm = {
                pointDeVie: 0, pa: 0, pm: 0, pw: 0,
                maitriseFeu: 0, maitriseEau: 0, maitriseTerre: 0, maitriseAir: 0,
                resistancesFeu: 0, resistancesEau: 0, resistancesTerre: 0, resistancesAir: 0,
                di: 0, soinsRealises: 0, cc: 0, parade: 0, initiative: 0, portee: 0,
                esquive: 0, tacle: 0, sagesse: 0, prospection: 0, volonte: 0,
                maitriseCritique: 0, resistancesCritique: 0, maitriseDos: 0, resistancesDos: 0,
                maitriseMelee: 0, armureDonnee: 0, maitriseDistance: 0, armureRecue: 0,
                maitriseSoins: 0, DIindirect: 0, maitriseBerzerk: 0
            };
            ...
            parts.forEach(part => {
                ...
                if (!isNaN(index) && !isNaN(val) && index >= 0 && index < keys.length) {
                    aptitudesManual[keys[index]] = val;
                }
            });
